...
class ApiClient {
  ...
  constructor() {
    ...
    this.baseUrl = 'https://backend-1-icoe.onrender.com'
  }

  async sendChatMessage(message: string, sessionId?: string): Promise<ChatResponse> {
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 30000000); // 30 second timeout
      ...
      if (!response.ok) {
        ...
        if (response.status === 400) {
          ...
          return { error: `Embedding not complete(Ratio completion 70%) ${response.status} - ${response.statusText}` };
        }
      }
